...
#
def recuperation_extension(liste):
    split_fichier = [(element, element.split(".")[-1].lower()) for element in liste]
    return split_fichier


def recuperation_definition(extensions, definitions):
...
split_extensions = (recuperation_extension(fichiers))

for extension in split_extensions:
    definition = recuperation_definition(extension[1], definition_extensions)
    if not definition:
        print("Extension inconnue ou introuvable")
    else:
        print(f"{extension[0]} est {definition}")
#
# # Correction
